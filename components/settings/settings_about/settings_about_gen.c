...
lv_obj_t * settings_about_create(lv_obj_t * parent)
{
    ...
    if (helios_ui_check_target(HELIOS_UI_TARGET_ALL)) {
        ...
        lv_label_set_translation_tag(hs_text_normal_0, "about");
        ...
        lv_obj_t * hs_row_0 = hs_row_create(hs_card_1);
        lv_obj_set_style_pad_column(hs_row_0, 10, 0);
        lv_obj_t * hs_text_normal_1 = hs_text_normal_create(hs_row_0);
        lv_label_set_text(hs_text_normal_1, "Chronos\nHelios");

        lv_obj_t * hs_text_small_0 = hs_text_small_create(hs_card_1);
        lv_obj_set_width(hs_text_small_0, lv_pct(100));
        lv_label_set_translation_tag(hs_text_small_0, "helios_info");

        hs_line_create(hs_card_1);

        hs_info_create(hs_card_1, "", "", "chronos.ke/helios");

        lv_obj_t * hs_card_2 = hs_card_create(wd_list_container_0);
        hs_info_sb_sb_create(hs_card_2, &sb_board_oem, &sb_board_name);

        hs_line_create(hs_card_2);

        hs_info_sb_sb_create(hs_card_2, &sb_board_type, &sb_board_mac);

        hs_line_create(hs_card_2);

        hs_info_sb_create(hs_card_2, "Label", "ram", &sb_board_ram, "%s");

        hs_info_sb_create(hs_card_2, "Label", "psram", &sb_board_psram, "%s");

        hs_line_create(hs_card_2);

        hs_info_sb_create(hs_card_2, "Label", "flash", &sb_board_flash, "%s");

        hs_line_create(hs_card_2);

        hs_info_sb_create(hs_card_2, "Label", "display", &sb_screen_res, "%s");

        hs_info_sb_create(hs_card_2, "Label", "type", &sb_display_type, "%s");

        lv_obj_t * hs_card_3 = hs_card_create(wd_list_container_0);
        hs_info_sb_create(hs_card_3, "Label", "firmware", &sb_firmware_version, "%s");

        hs_info_create(hs_card_3, "Helios UI", "", UI_VERSION);

        hs_info_sb_create(hs_card_3, "LVGL", "", &sb_lvgl_version, "%s");

        hs_line_create(hs_card_3);

        hs_info_sb_create(hs_card_3, "ChronosApp", "", &sb_chronos_app_version, "%s");

        hs_info_sb_create(hs_card_3, "ChronosESP", "", &sb_chronos_esp_version, "%s");
