...
lv_obj_t * widgets_create(void)
{
    ...
    if (helios_ui_check_target(HELIOS_UI_TARGET_ALL)) {
        ...
        lv_obj_t * wd_panel_0 = wd_panel_create(lv_obj_0);
        lv_obj_t * wd_panel_container_0 = wd_panel_get_container(wd_panel_0);
        lv_obj_set_scroll_snap_x(wd_panel_container_0, LV_SCROLL_SNAP_CENTER);
        lv_obj_add_style(wd_panel_container_0, &style_cont, 0);
        weather_widget_create(wd_panel_container_0);

        music_widget_create(wd_panel_container_0);
